...
void Menu::Initialize(World* _world, IDirect3DDevice9Ptr m_Device, LPVOID* m_game)
{
	...
	this->BgTex = game->resource->CreateTexture(1,BgID,false);
	this->FgTex = game->resource->CreateTexture(1,FgID,true);
	AccountCreateTexture = game->resource->CreateTexture(1,12,true).Texture;
	MenuButtonTexture = game->resource->CreateTexture(1,13,true).Texture;
	LoginModificationTexture = game->resource->CreateTexture(1,14,true).Texture;
	LoginButtonTexture = game->resource->CreateTexture(1,15,true).Texture;
	CharacterModificationTexture = game->resource->CreateTexture(1,22,false).Texture;
	this->LoginBoxTex = game->resource->CreateTexture(1,2,false);
	this->CharacterBox = game->resource->CreateTexture(1,11,false);
	this->LoginFgTex = game->resource->CreateTexture(1,LoginFgID,true);
	this->CreateCharTex = game->resource->CreateTexture(1,20,false);
	this->ChangePassTex = game->resource->CreateTexture(1,21,false);
...
void Menu::Render()
{
		...
		switch(game->Stage)
		{
			case(game->PMenu):
				{
					game->Draw(this->Sprite,this->FgTex.Texture,230, 480 - game->resource->GetImageInfo(1,FgID, true).Height - 10,D3DCOLOR_ARGB(255,255,255,255) );
			...
			case(game->PCreateAccount):
				{
					game->Draw(this->Sprite,this->LoginFgTex.Texture,30, 480 - game->resource->GetImageInfo(1,LoginFgID, true).Height - 10,D3DCOLOR_ARGB(255,255,255,255) );
