...
void CharacterModel::Initialize(LPVOID* m_game)
{
	...
	this->yoffset = 0;
	_game = (Game*)m_game;
	CharacterStandTexture = _game->ResourceManager->CreateTexture(8, 1, true).Texture;
	CharacterWalkTexture = _game->ResourceManager->CreateTexture(8, 2, true).Texture;
	CharacterAttackBluntTexture = _game->ResourceManager->CreateTexture(8, 3, true).Texture;
	CharacterSpellTexture = _game->ResourceManager->CreateTexture(8, 4, true).Texture;
	CharacterSitChairTexture = _game->ResourceManager->CreateTexture(8, 5, true).Texture;
	CharacterSitGroundTexture = _game->ResourceManager->CreateTexture(8, 6, true).Texture;
	CharacterAttackBowTexture = _game->ResourceManager->CreateTexture(8, 7, true).Texture;
	CharacterEmoteTexture = _game->ResourceManager->CreateTexture(8, 8, true).Texture;
	...
	for (int itemlayer = 0; itemlayer < 8; itemlayer++)
	{
		...
		switch (layerinfo.Layer_Type)
		{
			...
			case(LayerType::BackHair):
			{
				...
				_Sprite->SetTransform(&mat);
				_Sprite->Draw(_game->ResourceManager->CreateTexture(layerinfo.file - this->Gender, TextureIndex, true).Texture.get(), NULL, Center, Pos, m_Color);
				...
				if (this->direction == 0 || this->direction == 2)
				{
					if (this->Stance == PlayerStance::BluntAttacking)
					{
						if (this->frame_ID == 1)
						{
							...
							_Sprite->Draw(_game->ResourceManager->CreateTexture(layerinfo.file - this->Gender, Addint + 1 + IndexOffSet  + (this->WeaponID * 100) +4, true).Texture.get(), NULL, Center, Posx, m_Color);
							delete Posx;
						}
					}
					if (this->Stance == PlayerStance::BowAttacking)
