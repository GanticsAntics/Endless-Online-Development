...
Map_UI_Inventory::Map_UI_Inventory(void* m_UIElement, void* m_Game)
{
	...
	this->UI_Element_InventoryPPdoll = new Button(m_Game, 486, 340, 38, 385, 88, 19, false, Ptr_Inv_Game->resource->CreateTexture(2, 27, false).Texture);

	this->UI_Element_InventoryDrop = new Button(m_Game, 491, 399, 0, 15, 38, 37, true, Ptr_Inv_Game->resource->CreateTexture(2, 27, false).Texture);
	this->UI_Element_InventoryJunk = new Button(m_Game, 531, 399, 0, 89, 38, 37, true, Ptr_Inv_Game->resource->CreateTexture(2, 27, false).Texture);
...
void Map_UI_Inventory::Render()
{
	...
	case(Map_UI::UI_ElementStage::UI_Element_Inventory):
	{
		Ptr_Inv_Game->Draw(Ptr_Inv_MapUI->Sprite, Ptr_Inv_Game->resource->CreateTexture(2, 44, false).Texture, 102, 332, 0.1f, D3DCOLOR_ARGB(255, 255, 255, 255));
		...
		for each (InventoryGridCell Cell in this->InventoryGrid)
		{
			...
			Ptr_Inv_MapUI->Sprite->Draw(Ptr_Inv_Game->resource->CreateTexture(2, 27, true).Texture.get(), &SrcRect, Center, Pos, col);
		...
		for each (InventoryGridCell Cell in this->InventoryGrid)
		{
			...
			if (it == ItemIDS.end())
			{
				...
				{
					int m_itemHeight = Ptr_Inv_Game->resource->GetImageInfo(23, m_item.graphic * 2, true).Height;
					int m_itemWidth = Ptr_Inv_Game->resource->GetImageInfo(23, m_item.graphic * 2, true).Width;

					itemPos->x = Ptr_Inv_Game->MouseX - m_itemWidth / 2;
					itemPos->y = Ptr_Inv_Game->MouseY - m_itemHeight / 2;
					Ptr_Inv_MapUI->Sprite->Draw(Ptr_Inv_Game->resource->CreateTexture(23, m_item.graphic * 2, true).Texture.get(), NULL, itemCenter, itemPos, D3DCOLOR_ARGB(150, 255, 255, 255));
				}
				else
				{
					Ptr_Inv_MapUI->Sprite->Draw(Ptr_Inv_Game->resource->CreateTexture(23, m_item.graphic * 2, true).Texture.get(), NULL, itemCenter, itemPos, D3DCOLOR_ARGB(255, 255, 255, 255));
				}
...
void Map_UI_Inventory::RenderPaperdoll()
{
	if (this->paperdoll._PlayerID >= 0)
	{
		...
		for (int i = 0; i < 15; i++)
		{
			...
			Ptr_Inv_MapUI->Sprite->Draw(Ptr_Inv_Game->resource->CreateTexture(23, m_item.graphic * 2, true).Texture.get(), NULL, Centre, Pos, D3DCOLOR_ARGB(255, 255, 255, 255));
		}
